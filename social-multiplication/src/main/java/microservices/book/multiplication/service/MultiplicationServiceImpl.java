...
class MultiplicationServiceImpl implements MultiplicationService {
	...
	public boolean checkAttempt(final MultiplicationResultAttempt attempt) {
		...
		log.info("------------ MultiplicationServiceImpl : checkAttempt START ------------");
		...
		log.info("------------ MultiplicationServiceImpl : checkAttempt : 이벤트로 결과를 전송 ------------");
		
		// 이벤트로 결과를 전송
		eventDispatcher.send(new MultiplicationSolvedEvent(checkedAttempt.getId(), checkedAttempt.getUser().getId(),
				checkedAttempt.isCorrect()));

		log.info("------------ MultiplicationServiceImpl : checkAttempt END ------------");
